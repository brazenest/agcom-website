...
export const queryApi = async ({ endpoint, params = [], method = 'GET', data = {} }: queryApiParamsT) => {
    ...
    switch (method) {
        case 'GET':
            const getMethodQueryResponse = await fetch(requestUrl);
            response = await getMethodQueryResponse.json();
            break;

        case 'POST':
            const postMethodQueryResponse = await fetch(requestUrl, {
                method: 'POST',
                headers: {
            ...
            response = await postMethodQueryResponse.json();
            break;
