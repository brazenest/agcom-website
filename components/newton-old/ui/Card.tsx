...
export const Card = ({ badge, image, heading, text, bullets, className, children }: CardProps) => {
	...
	return (
		<div
			className={cn("named-card aspect-card", "bg-bg-subtle p-3 border border-border rounded-xl shadow-sm", className)}>
			...
			{image && (
				<span className="image-wrap w-full aspect-[4/5] overflow-hidden rounded-lg">
					<NextImage
						src={image.src}
						alt={image.alt}
						width={image.width}
			...
			{heading && (
				<CardHeading level={3} className="font-heading font-semibold text-lg md:text-xl mt-4.5 mb-3">
					{heading}
				</CardHeading>
			)}
